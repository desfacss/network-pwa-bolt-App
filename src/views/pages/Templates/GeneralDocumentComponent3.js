      ...
      <div style={{ display: 'flex', justifyContent: alignment, marginBottom: '10px' }}>
        ...
        {/* {field.type === 'display' && (
          <Form.Item name={field.key} noStyle>
            {renderField(field, <span style={{ fontWeight: field.bold ? 'bold' : 'normal' }}>{form.getFieldValue(field.key) || field.label}</span>)}
          </Form.Item>
        )} */}
        {field.type === 'display' && (
          <Form.Item name={field.key} noStyle>
  ...
  const updateSummary = (data) => {
    ...
    if (summarySection && summarySection.fields) {
      ...
      summarySection.fields.forEach(field => {
        ...
        } else if (field.key === 'tax' || field.key === 'gst') {
          // Assuming a 10% tax rate; adjust if needed dynamically
          const taxRate = field.key === 'gst' ? 0.18 : 0.1; // 18% for GST, 10% for others
          newValues[field.key] = (parseFloat(newValues.subtotal || 0) * taxRate).toFixed(2);
        } else if (field.key === 'total' || field.key === 'grandTotal') {
          // Total includes subtotal + tax
          const total = parseFloat(newValues.subtotal || 0) + parseFloat(newValues.tax || newValues.gst || 0);
          newValues[field.key] = total.toFixed(2);
        } else if (field.key === 'roundedOff') {
          // Round off calculation - this might need to be adjusted based on exact rules
          const grandTotal = parseFloat(newValues.total || newValues.grandTotal || 0);
          newValues[field.key] = (Math.round(grandTotal) - grandTotal).toFixed(2);
        }
      });
