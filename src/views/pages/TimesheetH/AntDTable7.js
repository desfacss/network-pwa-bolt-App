...
const Timesheet = () => {
    ...
    const [disabled, setDisabled] = useState(false); // Default to Weekly
    const [currentDate, setCurrentDate] = useState(getMonday(new Date()));
    const [existingTimesheetId, setExistingTimesheetId] = useState(null);
    const [expandedRows, setExpandedRows] = useState({});

    const handleToggleDescription = (rowKey) => {
        setExpandedRows((prev) => ({
            ...prev,
            [rowKey]: !prev[rowKey], // Toggle the expanded state
        }));
    };
    ...
    const isCurrentDateDisabled = () => {
        ...
        if (viewMode === 'Weekly') {
            // Disable all dates except the current week's Monday
            return currentDate.getTime() > currentMonday.getTime();
        } else if (viewMode === 'Monthly') {
            // Disable all dates except the current month's first day
    ...
    const columns = [
        {
            title: 'Project',
            // title: (
            //     <div style={{ writingMode: 'vertical-lr', textOrientation: 'upright', whiteSpace: 'pre' }}>
            //         Project
            //     </div>
            // ),
            dataIndex: 'project',
            ...
            render: (_, record, index) => (
                <Space align="start" style={{ alignItems: 'flex-start', verticalAlign: 'top' }}>
                    <Button style={{ verticalAlign: 'top' }}
                        type="link" onClick={() => handleToggleDescription(record.key)}
                    // onClick={() => {
                    //     const newData = [...dataSource];
