...
const PERTInput = ({ optimistic, likely, pessimistic, aspirational, reasons, timeDisplay, onChange, onUnitChange }) => {
  return (
    <Space>
      <InputNumber
        value={optimistic}
...
const ProcessEditor = ({ initialData }) => {
    ...
    if (selectedItem.type === 'step') {
      ...
      Object.assign(task, {
        ...
        pert_time: {
          leadTime: {
            ...values.leadTime,
            timeUnit: 'minutes', // Always save as minutes
            timeDisplay: values.leadTime.timeDisplay,
          },
          effortTime: {
            ...values.effortTime,
            timeUnit: 'minutes', // Always save as minutes
            timeDisplay: values.effortTime.timeDisplay,
          }
        },
        pert_cost: {
  ...
  //   });
  // };
  const handleUnitChange = (value, timeType) => {
    const values = form.getFieldsValue();
    const currentValues = values[timeType];
    const newValues = {
      ...currentValues,
      optimistic: convertToUnit(currentValues.optimistic, currentValues.timeDisplay, value),
      likely: convertToUnit(currentValues.likely, currentValues.timeDisplay, value),
      pessimistic: convertToUnit(currentValues.pessimistic, currentValues.timeDisplay, value),
      aspirational: convertToUnit(currentValues.aspirational, currentValues.timeDisplay, value),
      timeDisplay: value
    };
    form.setFieldsValue({ [timeType]: newValues });
  };
  ...
  const setInitialValues = (item) => {
    if (item.type === 'task') {
      ...
      form.setFieldsValue({
        ...item,
        leadTime: {
          ...item.pert_time.leadTime,
          optimistic: minutesToUnit(item.pert_time.leadTime.optimistic, unit),
          likely: minutesToUnit(item.pert_time.leadTime.likely, unit),
          pessimistic: minutesToUnit(item.pert_time.leadTime.pessimistic, unit),
          timeDisplay: unit,
        },
        effortTime: {
          ...item.pert_time.effortTime,
          optimistic: minutesToUnit(item.pert_time.effortTime.optimistic, unit),
          likely: minutesToUnit(item.pert_time.effortTime.likely, unit),
          pessimistic: minutesToUnit(item.pert_time.effortTime.pessimistic, unit),
          timeDisplay: unit,
        },
            ...
            <>
              ...
              {/* <div className="form-section">
                ...
                {['leadTime', 'effortTime'].map((timeType) => (
                  <Form.Item key={timeType} label={`${timeType.charAt(0).toUpperCase() + timeType.slice(1)}`}>
                    <PERTInput
                      optimistic={form.getFieldValue(`${timeType}.optimistic`) || 0}
                      likely={form.getFieldValue(`${timeType}.likely`) || 0}
                      pessimistic={form.getFieldValue(`${timeType}.pessimistic`) || 0}
                      aspirational={form.getFieldValue(`${timeType}.aspirational`) || 0}
                      reasons={form.getFieldValue(`${timeType}.reasons`) || []}
                      timeDisplay={form.getFieldValue(`${timeType}.timeDisplay`) || 'days'}
                      onChange={(field, value) => {
                        form.setFieldsValue({ [timeType]: { ...form.getFieldValue(timeType), [field]: value } });
                      }}
                    />
                  </Form.Item>
                ))}
                ...
                {['leadTime', 'effortTime'].map((timeType) => (
                  <Form.Item key={timeType} label={`${timeType.charAt(0).toUpperCase() + timeType.slice(1)}`}>
                    <PERTInput
                      ...
                      reasons={form.getFieldValue([timeType, 'reasons']) || []}
                      timeDisplay={form.getFieldValue([timeType, 'timeDisplay']) || 'days'}
                      onChange={(field, value) => form.setFieldsValue({ [timeType]: { ...form.getFieldValue(timeType), [field]: value } })}
                      onUnitChange={(value) => handleUnitChange(value, timeType)}
                    />
                  </Form.Item>
                ))}
              </div>
