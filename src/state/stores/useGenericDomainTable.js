...
const createTableStore = () => create(
    persist(
        (set, get) => ({
            ...
            pagination: { currentPage: 1, pageSize: 5 }, // Changed 'current' to 'currentPage'
            syncStatus: {
                pendingOperations: 0,
                lastSync: null,
                hasConflicts: false,
        ...
        }),
        {
            ...
            getStorage: () => ({
                ...
                getItem: async (name) => {
                    try {
                        const value = await offlineDB.table('syncQueue').get(name);
                        console.log('1. OFFLINEDB: Stored state:', value); // Log what's being fetched
                        return value ? JSON.parse(value) : null;
                    } catch (error) {
                        console.error('2. OFFLINEDB: Storage read error:', error);
                        return null;
                    }
                },
