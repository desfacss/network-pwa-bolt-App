...
//   const [deleteModalVisible, setDeleteModalVisible] = useState(false);
//   const [messageToDelete, setMessageToDelete] = useState(null);
//   const [likes, setLikes] = useState({}); // State to track likes (client-side for demo)
//   const { session } = useSelector((state) => state.auth);
//   const queryClient = useQueryClient();

//   // Fetch categories
//   const { data: idToNameMap = new Map() } = useQuery({
//     queryKey: ['ib_categories'],
//     queryFn: async () => {
...
//     queryKey: ['channel_posts', channel_id, session?.user?.id, isPrivate],
//     queryFn: async () => {
//       let query = supabase
//         .from('channel_posts')
//         .select(
...
//     },
//     staleTime: 5 * 60 * 1000,
//     enabled: !!channel_id && !!session?.user?.id,
//     onSuccess: (data) => {
//       // Initialize likes (client-side for demo)
//       const initialLikes = {};
//       data.forEach((item) => {
//         initialLikes[item.id] = 0; // Replace with backend data if available
//       });
//       setLikes(initialLikes);
//     },
//   });
...
//   // Fetch unread counts
//   const { data: unreadPostCounts = {} } = useQuery({
//     queryKey: ['unread_counts', session?.user?.id, channel_id],
//     queryFn: async () => {
...
//         const messageContent = message.message?.toLowerCase() || '';
//         const tags =
//           message.details?.tags
//             ?.map((tag) => idToNameMap.get(tag)?.toLowerCase() || '')
//             .join(' ') || '';
//         const category = idToNameMap.get(message.details?.category_id)?.toLowerCase() || '';
//         return (
//           userName.includes(searchText.toLowerCase()) ||
//           messageContent.includes(searchText.toLowerCase()) ||
...
//     },
//     onSuccess: () => {
//       queryClient.invalidateQueries(['channel_posts', channel_id]);
//       message.success('Message deleted successfully.');
//     },
//     onError: (err) => {
//       console.error('Error deleting message:', err);
...
//   const handleSubmit = (data, isUpdate) => {
//     if (isUpdate) {
//       queryClient.setQueryData(['channel_posts', channel_id, session?.user?.id, isPrivate], (old) =>
//         old.map((msg) => (msg.id === data.id ? data : msg))
//       );
//     } else {
//       queryClient.setQueryData(['channel_posts', channel_id, session?.user?.id, isPrivate], (old) => [
//         data,
...
//           form={form}
//           onSubmit={handleSubmit}
//           isSubmitting={false} // isSubmitting removed for simplicity
//           session={session}
//         />
//       )}
...
//                   )}
//                   {item.details?.category_id && (
//                     <Tag>{idToNameMap.get(item.details.category_id) || item.details.category_id}</Tag>
//                   )}
//                   {item.details?.tags?.map((tag) => (
...
//         title="Delete Post"
//         open={deleteModalVisible}
//         onOk={() => deleteMutation.mutate(messageToDelete)}
//         onCancel={() => setDeleteModalVisible(false)}
//         okText="Yes"
//         cancelText="No"
//         centered
...
//             type="primary"
//             danger
//             onClick={() => deleteMutation.mutate(messageToDelete)}
//             style={{ width: '45%' }}
//             size="large"
//           >
//             Yes
...
const ForumComment = ({
  ...
  const [allMessages, setAllMessages] = useState([]); // Store all fetched posts
  const { session } = useSelector((state) => state.auth);
  const queryClient = useQueryClient();
  ...
  // Fetch posts (lazy loading with pagination)
  const { data: newMessages = [], isLoading, isFetching } = useQuery({
    queryKey: ['channel_posts', channel_id, session?.user?.id, isInbox, page], // Include page in queryKey
    queryFn: async () => {
      let query = supabase
        .from('channel_posts')
        .select(
      ...
      const { data, error, count } = await query
        .order('inserted_at', { ascending: false }) // Sort by inserted_at, most recent first
        .range(from, to); // Fetch only 5 posts
  ...
  useEffect(() => {
    if (newMessages.length > 0) {
      ...
      // Initialize likes for new messages
      setLikes((prevLikes) => {
        const initialLikes = {};
        newMessages.forEach((item) => {
          initialLikes[item.id] = prevLikes[item.id] || 0; // Preserve existing likes
        });
        return { ...prevLikes, ...initialLikes };
      });
    }
  }, [newMessages]); // Removed 'likes' from dependencies
  ...
  const filteredMessages = searchText
    ? allMessages.filter((message) => {
        ...
        const tags =
          message.details?.tags
            ?. VGAmap((tag) => idToNameMap.get(tag)?.toLowerCase() || '')
            .join(' ') || '';
        const category = idToNameMap.get(message.details?.category_id)?.toLowerCase() || '';
        return (
          userName.includes(searchText.toLowerCase()) ||
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['channel_posts', channel_id]);
      // Remove deleted message from allMessages
  ...
  const handleSubmit = (data, isUpdate) => {
    if (isUpdate) {
      setAllMessages((prev) => prev.map((msg) => (msg.id === data.id ? data : msg)));
    } else {
